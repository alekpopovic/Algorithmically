  ...
  module Evolutionary
    class Genetic
      ...
      def random_bitstring(num_bits)
        (0...num_bits).inject('') { |s, _i| s << (rand < 0.5 ? '1' : '0') }
      end
      ...
      def search(max_gens, num_bits, pop_size, p_crossover, p_mutation)
        population = Array.new(pop_size) do |_i|
          { bitstring: random_bitstring(num_bits) }
        end
        population.each { |c| c[:fitness] = onemax(c[:bitstring]) }
        best = population.min { |x, y| y[:fitness] <=> x[:fitness] }
        max_gens.times do |gen|
          selected = Array.new(pop_size) { |_i| binary_tournament(population) }
